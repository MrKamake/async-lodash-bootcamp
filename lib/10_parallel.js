...
export default function parallel(collection, callback) {
  ...
  if (!collection[collectionKeys[0]]) {
    Array.isArray(collection) ? callback(null, resultArr) : callback(null, resultObj);
  }
  ...
  for (let i = 0; i < collectionKeys.length; i++) {
    collection[collectionKeys[i]]((err, ...result) => {
      ...
      if (Array.isArray(collection)) {
        ([...result].length === 1) ? resultArr[i] = (result[0]) : resultArr[i] = [...result];
      } else {
        ([...result].length === 1) ? resultObj[collectionKeys[i]] = result[0] : resultObj[collectionKeys[i]] = [...result];
      }
